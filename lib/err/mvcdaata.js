...
(function() {
  ...
  const EMBOS_BASE = 'https://vaplayer.ru';
  ...
  function playEpisode(imdbId, season, episode) {
    ...
    const url = `${EMBOS_BASE}/embed/tv/${imdbId}/${season}/${episode}?skin=netflix&color=ffa500&title=false`;
    const playerWrapper = document.getElementById('playerWrapper');
    if (playerWrapper) {
  ...
  function unlockPlayer() {
    ...
    if (currentItem?.type === 'movie') {
      const url = `${EMBOS_BASE}/embed/movie/${imdb}?skin=netflix&color=ffa500&title=false`;
      const playerWrapper = document.getElementById('playerWrapper');
      if (playerWrapper) {
        playerWrapper.innerHTML = `<iframe src="${url}" allowfullscreen allow="autoplay; encrypted-media" loading="lazy"></iframe>`;
      }
