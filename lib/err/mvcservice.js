...
(function() {
  ...
  const EMBOS_BASE = 'https://embos.top';
  const SMARTLINK = 'https://www.profitablecpmratenetwork.com/mb3prdha1m?key=726191ab470a089d7f1b0f18c3e14b8f';
  ...
  function playEpisode(imdbId, season, episode) {
    ...
    const url = `${EMBOS_BASE}/tv/?mid=${imdbId}&s=${season}&e=${episode}`;
    const playerWrapper = document.getElementById('playerWrapper');
    if (playerWrapper) {
  ...
  function unlockPlayer() {
    ...
    if (currentItem?.type === 'movie') {
      const url = `${EMBOS_BASE}/movie/?mid=${imdb}`;
      const playerWrapper = document.getElementById('playerWrapper');
      if (playerWrapper) {
        playerWrapper.innerHTML = `<iframe src="${url}" allowfullscreen allow="autoplay; encrypted-media" loading="lazy"></iframe>`;
      }
